...
#if defined (MAKE_BCAST_LIBRARY_) && defined (_WIN32) // for windows ...
...
{
public:
    callback(std::string listener_id_, boost::function<void(const derived&)> cb_)
    {
        _cb = cb_;
        _listener_id = listener_id_;
    }

    boost::function<void(const derived&)> _cb;
};
...
public:
    ...
    template <typename derived>
    key add(boost::function<void(const derived&)> cb_, std::string event_name, std::string listener_id)
    {
        ev_cb_map_iter it;
